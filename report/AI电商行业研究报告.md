...
**三、站内AI导购在「个体层相对效果」这一构造上只有一个公开数据点。** 在本报告的检索范围内（`docs/05` 所列的一手来源、第三方监测机构与严肃媒体，数据截至2026年7月），以「使用者对未使用者」为对照的效果数据仅有Amazon Rufus的「使用者购买完成率高约60%」（亚马逊2025Q4财报电话会，观察性对比）。亚马逊另披露的「增量年化销售近120亿美元」是绝对额、归因方法未完整披露，不可与其他案例横向比较，也不能替代相对效果证据。这既说明该方向的公开证据基础很薄，也说明自建随机对照实验是当前最可靠、且在本报告检索范围内唯一可自主取得的识别途径；分阶段放量亦可支撑双重差分等准实验识别，但需额外的平行趋势假设。
...
| 1.5 | +0.244 | +0.190 | 1.28 | 2.77 |
| **1.96（临界点一）** | +0.249 | +0.249 | **1.00** | 2.15 |
| 2.0 | +0.122 | +0.127 | 0.96 | 2.07 |
| **2.75（临界点二）** | +0.162 | +0.349 | 0.46 | **1.00** |
| 3.0 | +0.067 | +0.190 | 0.35 | 0.76 |

（表内增益的绝对值随 u 与 s 的具体组合而异，但增益比与效率比只随乘积变化，故按 u×s 索引。）
...
**四、站内AI导购在「个体层相对效果」构造上只有一个公开数据点，且是观察性的。** 图9面板①只有Rufus一根柱。Salesforce的企业分组数据与Adobe、Shopify的渠道数据分属另两种对照构造，不能替代；亚马逊的「增量年化销售」为绝对额且归因方法未完整披露，不可跨案例比较。这意味着建立随机对照实验能力（设计见§6.2建议二）不是可选项——它是当前最可靠、且在本报告检索范围内唯一可自主取得的识别途径；分阶段放量可作为退路支撑双重差分等准实验识别，但需额外的平行趋势假设。
...
| u×s 区间 | 本建议的可用程度 |
|---|---|
| ＜1.96 | 全部可用：可切换指标，且「优先提高每轮产出」的路径排序成立 |
| 1.96 至 2.75 | 可切换指标；路径排序改按单位推理成本判定（此区间内仍为路径A占优） |
| 2.75 至 3.5 | 可切换指标；但**路径排序反转，应优先增加对话轮次**（见§1.8分析五） |
| ≥3.5 | 不可用：「差距落在交互层」这一前提本身不成立，须转入§1.6限制①的意图构成检验 |
...
| 主指标1（二值） | 观测窗口内至少有一次会话进入商品详情页的**用户占比**（用户级比率，与随机化单元一致），基线约10% | 以人均IPV 0.11次/人·日近似推得。**重述条款**：该基线同样以未校正的0.11为依据，u测定后须按0.11u重算基线与样本量（若u＝2则基线约20%、每组样本量降至约1570人） |
...
| 护栏指标 | 客诉率相对提升的95%置信区间上界 ≤ +10%。**分母定义为观测窗口内的去重用户数**（与随机化单元一致），非人·日 | 采用非劣性边界，避免「高于基线即不放量」在随机波动下等价于「大概率不放量」。按去重用户计，1%分流×14日每组约2.5万人，若基线率低于0.5%则不足以对+10%边界定功（需约31万人／组），此时改用绝对增量阈值 |
