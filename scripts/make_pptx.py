...
def new_slide(prs, title=None, kicker=None, page_no=None, title_color=INK):
    ...
    add_textbox(slide, Inches(0.55), Inches(7.08), Inches(12.2), Inches(0.35),
                [("沃尔玛中国 CTO 汇报｜AI 电商行业研究（预读版 v1.2）｜数据截至 2026-07｜内部资料·注意保密"
...
def build():
    ...
    add_textbox(slide, Inches(1.25), Inches(4.35), Inches(11.0), Inches(2.2), [
        ...
        ("版本：预读版 v1.2 ｜ 数据截至 2026-07 ｜ 配套预读文档 30 分钟", 12.5, False, RGBColor(0x93, 0xC5, 0xFD), None),
    ...
    table_slide(prs, "分析坐标系", "AI 购物能力五级：山姆站内做 A1→A3、站外做 A2、A4 只在复购场景灰度", pn,
                ...
                [
                    ...
                    ["A5", "自主代理", "长期授权、全网比价代买", "尚无规模化案例（美国信任全自动结账者仅约 14%，行业调查汇总口径·中置信）"],
                ],
                [0.8, 1.8, 3.8, 5.6],
    ...
    # ---- 13 站内导购成绩单 ----
    pn += 1
    image_slide(prs, "中国战场 4/4", "两条证据线都为正：站内导购 +59%~60%；站外 AI 引荐转化优势 +42%~54%",
                "07_instore_ai_scorecard.png", pn,
                "决策含义：站内导购与站外引荐口径不同、分开呈现（不合并为单一区间）；京东斜纹柱不参与判断——结论不依赖低置信数据。", img_h=4.5)
    ...
    table_slide(prs, "行动方案", "90 天三个试点 + 一条谈判线：每项带 KPI 门槛与止损条件", pn,
                ...
                [0.6, 2.2, 3.6, 3.3, 2.6],
                "资源诉求（CTO 决策）：8~12 人虚拟团队（90 天约 24~36 人月）；国内合规模型栈选型（混元/DeepSeek/通义）；第三方数据约百万元级/年；首期 90 天总预算量级=数百万元级人民币（立项讨论用估算，财务核价出准数）。",
