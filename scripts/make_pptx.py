...
def new_slide(prs, title=None, kicker=None, page_no=None, title_color=INK):
    ...
    add_textbox(slide, Inches(0.55), Inches(7.08), Inches(12.2), Inches(0.35),
                [("沃尔玛中国 CTO 汇报｜AI 电商行业研究（预读版 v1.1）｜数据截至 2026-07｜内部资料·注意保密"
...
def build():
    ...
    add_textbox(slide, Inches(1.25), Inches(4.35), Inches(11.0), Inches(2.2), [
        ...
        ("版本：预读版 v1.1 ｜ 数据截至 2026-07 ｜ 配套预读文档 30 分钟", 12.5, False, RGBColor(0x93, 0xC5, 0xFD), None),
    ...
    table_slide(prs, "分析坐标系", "AI 购物能力五级：山姆站内做 A1→A3、站外做 A2、A4 只在复购场景灰度", pn,
                ...
                [
                    ...
                    ["A5", "自主代理", "长期授权、全网比价代买", "尚无规模化案例（美国仅 14% 消费者信任全自动结账）"],
                ],
                [0.8, 1.8, 3.8, 5.6],
    ...
    # ---- 10 中国格局 ----
    pn += 1
    table_slide(prs, "中国战场 1/4", "入口格局：五类玩家已就位，微信系联盟模式与沃尔玛通道天然契合", pn,
    ...
    image_slide(prs, "中国战场 4/4", "站内 AI 导购已被证明有效：转化提升 40%~60% 是可信区间",
                ...
                "决策含义：可信区间取自绿色（高/中置信）指标；京东斜纹柱不参与区间判断——结论不依赖低置信数据。", img_h=4.55)
    ...
    threat_lines = [
        ...
        ("T4 供给侧标准缺位（1~2 年）：feed 不做 LLM 就绪化，可见度被动下降（海外：34% 商品页对 AI 不可见）→ 对应 P2", 12, False, INK, 0),
    ]
    ...
    note(slide, "对标：Instacart Cart Assistant（膳食规划）已落地 Kroger/Sprouts；A1~A5 能力分级定义见第 9 页与预读文档 §1.3。")
    ...
    table_slide(prs, "行动方案", "90 天三个试点 + 一条谈判线：每项带 KPI 门槛与止损条件", pn,
                ...
                [
                    ...
                    ["P2", "AI 可见度与平替风险基线", "500 组问题×豆包/千问/元宝/点点月测（含小象/叮咚 AI 动态）；500 SKU feed 改造 POC（设对照组）", "改造组被推荐率较对照组 +5pct 以上（连续两次月测同向）；T1 敞口收窄为实测", "无（纯情报与数据治理投入）"],
    ...
    bullets(slide, [
        ("生成式 AI 服务合规", "灰度是否触发备案义务=立项第一周法务定性（P1 首个里程碑）；备案材料 T+0 启动（法务+政府事务牵头，周期以数月计）；必要时灰度期提供『检索+模板』非生成式兜底模式"),
