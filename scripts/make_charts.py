...
def fig07_engagement():
    ...
    footer(fig,
           ...
           "「联合敏感度与反转边界（不可省略）」：②③是两个独立敞口，联合后交互层差距＝4.22÷(u×s)。"
           "u×s＝2时为2.11倍、＝3时为1.41倍、＝4.5时为0.94倍——「当u×s达到约3.5及以上时，该差距降至留存层差距（1.10～1.20倍）以下，方向反转」。"
           "同一乘积还决定改造路径的排序，且阈值更低：u×s≈1.96起「优先提高每轮产出」的增益优势消失，≈2.75起按单位推理成本计应改为优先增加对话轮次。"
           "三道阈值（1.96／2.75／3.5）构成同一量上的嵌套判据，完整表见报告§1.6与§1.8分析五。\n" + COMPILER)
