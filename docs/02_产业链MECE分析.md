...
| 关键数据 | Rufus 见上；淘宝 AI 导购内部口径实测 DAU 500 万、人均 IPV 0.11 次、人均对话轮次 1.3 轮（P3，见报告 §1.6）——瓶颈在决策深度而非流量；官方称 AI 万能搜「暂无商业化」，商业化启动时点是关键观察信号 |
...
**2d 硬件与 OS 入口（新兴）**：Alexa+（设备量级未交叉验证，不列数值）、千问 AI 眼镜、豆包手机、京东 JoyInside（植入量级为自报口径，不列数值）。观察点：语音天然适合复购型购物（生鲜/快消）。
...
| 收入/成本 | 收入=AI 渠道增量 GMV（AI 流量 RPV +37%）；成本=佣金/技术服务费（ChatGPT 约 4%+通道费；对比 Amazon 市场佣金约 15%，AI 渠道费率目前更低——费率演化是核心跟踪指标）、商品 feed 改造、GEO 预算增长（增幅数字未通过置信度门槛，不列数值）、让渡用户关系与数据的隐性成本 |
...
| 子层 | 代表 | 收入模式 / 关键数据 | 风险 |
|---|---|---|---|
| GEO/AEO 优化 | 海外：Profound（估值超 1 亿美元，监测 11 个 AI 面，含 Rufus；企业级 $2000+/月）、Peec、Otterly；SEO 转型：Semrush（被 Adobe 收购）、Ahrefs Brand Radar；国内：清蓝 PureblueAI（蓝色光标领投）、移山科技等 | 订阅 SaaS + 咨询服务；国内 GEO 服务需求随 AI 入口占比上升而增长（缺权威规模口径，不列数值）；Gartner：2028 年 50% 搜索流量转向 AI | 平台规则突变（AI 厂商自建广告位后，GEO 空间被压缩）；行业早期鱼龙混杂 |
...
| 内容生成与数字人 | 数字人直播服务（自报运营数据未达置信度门槛，不列示）、硅基智能等；全球数字人直播电商规模（艾媒转引，未取得原报告核实，不列数值） | 按坐席/时长订阅 | 同质化、平台对数字人直播的政策收紧 |
